...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB6:2310675 (update LAB5 steps)
        /*
...
void proc_run(struct proc_struct *proc)
{
    if (proc != current)
    {
        ...
        {
            // lab6:2310675 切换地址空间后刷新 TLB，避免使用旧 TLB 项
            current = proc;
            lsatp(proc->pgdir);
            flush_tlb();
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // lab6:2310675 step2: setup kernel stack
    if ((ret = setup_kstack(proc)) != 0)
    {
        goto bad_fork_cleanup_proc;
    }

    // lab6:2310675 step3: copy(or share) mm
    if ((ret = copy_mm(clone_flags, proc)) != 0)
    {
        goto bad_fork_cleanup_kstack;
    }

    // lab6:2310675 step4: setup thread context(trapframe/context)
...
load_icode(unsigned char *binary, size_t size)
{
     ...
     */
    // lab6:2310675 设置用户态入口与栈，并确保 sret 返回到 U-mode
    tf->gpr.sp = USTACKTOP;
    tf->epc = elf->e_entry;
